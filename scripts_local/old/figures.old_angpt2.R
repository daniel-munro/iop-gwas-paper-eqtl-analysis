...
p1 <- angpt2 |>
    ...
    theme_minimal() +
    # annotate("text", x = angpt2_tss - 0.03, y = 3, color = "#666666",
    #          label = "Angpt2\nTSS", hjust = 1) +
    annotate("text", x = angpt2_tss + 0.03, y = 3.25, color = "#666666",
...
p1b <- tyr |>
    ...
    ylab(expression(-log[10]*"p-value")) +
    theme_minimal() +
    annotate("text", x = tyr_tss - 0.07, y = 3.2, color = "#666666", label = "Tyr\nTSS",
             hjust = 1) +
    annotate("text", x = iop["1"] + 0.07, y = 3.6, color = iop_colors["1"], label = "IOP QTL",
             hjust = 0) +
...
p1c <- ndufaf6 |>
    ...
    ylab(expression(-log[10]*"p-value")) +
    theme_minimal() +
    annotate("text", x = ndufaf6_tss + 0.07, y = 3, color = "#666666", label = "Ndufaf6\nTSS",
             hjust = 0) +
    annotate("text", x = iop["5"] + 0.05, y = 3, color = iop_colors["5"], label = "IOP QTL",
             hjust = 0) +
...
plot_qtl_region_labels <- function(chr, posn) {
    iop_color <- iop_colors[as.character(chr)]
    perm |>
        mutate(pos = tss) |>
        filter(chrom == chr,
               pos > posn - window,
        ...
        mutate(pval = -log10(pval_nominal),
               ...
               #                gene, "")) |>
        ggplot(aes(x = pos, y = pval, label = gene)) +
        geom_segment(aes(x = pos, y = pval, xend = pos, yend = threshold),
                     size = 0.2, color = "#555555") +
        # geom_point(aes(y = log10_threshold), shape = "-", size = 3, color = "red") +
        geom_segment(aes(x = pos - 0.05, xend = pos + 0.05, y = threshold,
        ...
        geom_vline(xintercept = posn, color = iop_color, lty = 3) +
        annotate("text", x = posn + 0.03, y = 0.25, color = iop_color,
                 label = "IOP QTL", hjust = 0) +
        expand_limits(y = 0) +
...
p2 <- plot_qtl_region_labels(16, iop["16"])
p3 <- plot_qtl_region_labels(1, iop["1"])
p4 <- plot_qtl_region_labels(5, iop["5"])
