...
tss <- genes |>
    filter(gene %in% c("Tyr", "Ndufaf6", "Csmd1", "Angpt2")) |>
    select(gene, tss) |>
    deframe()
...
pairs <- tibble(gene_id = c("ENSRNOG00000016696", "ENSRNOG00000016421",
                            "ENSRNOG00000040040", "ENSRNOG00000030719")) |>
    group_by(gene_id) |>
    summarise(
...
ggplot(tmp2c, aes(x = pos, y = log10_pval)) +
    ...
    geom_richtext(data = tibble(a=1), x = tss["Tyr"] + 0.0, y = 5, color = "#666666",
                  label = "*Tyr* TSS", fill = NA, label.color = NA, hjust = 1) +
    annotate("text", x = iop["1"] + 0.03, y = 3, color = iop_colors["1"],
             label = "IOP QTL", hjust = 0) +
    geom_richtext(data = tibble(a=1), x = 152.5, y = 4.2, color = "blue",
...
ggplot(tmp3c, aes(x = pos, y = log10_pval)) +
    ...
    annotate("text", x = iop["5"] + 0.03, y = 3, color = iop_colors["5"],
             label = "IOP QTL", hjust = 0) +
    geom_richtext(data = tibble(a=1), x = 25, y = 3.8, color = "blue",
...
pairs |>
    ...
    annotate("text", x = iop["16"] + 0.03, y = 3, color = iop_colors["16"],
             label = "IOP QTL", hjust = 0) +
    geom_richtext(data = tibble(a=1), x = xmin["16"] + 0.2, y = 3.5, color = "blue",
