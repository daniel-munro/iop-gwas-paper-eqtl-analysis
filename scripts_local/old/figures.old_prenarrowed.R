...
p1 <- angpt2 %>%
    ...
    annotate("text", x = 75.92, y = 3, color = "#666666", label = "Angpt2\nTSS",
             hjust = 1) +
    annotate("text", x = 76.598844, y = 3, color = "red", label = "IOP QTL",
             hjust = 0) +
    annotate("text", x = 73.9, y = 3.95, color = "blue", label = "Angpt2 eQTL threshold",
             hjust = 0) +
    coord_cartesian(xlim = c(73.528844, 79.528844), expand = FALSE)
p1
...
plot_qtl_region_labels <- function(chr, posn) {
    ...
    perm %>%
        ...
        geom_vline(xintercept = posn, color = iop_color, lty = 3) +
        geom_point(size = 1) +
        ggrepel::geom_text_repel(force = 50, max.iter = 1e5,
...
p1 / p2 / p3 / p4 + plot_layout(heights = c(1, 1, 1, 1))
ggsave("figures/eQTL_figure.guide.png", width = 8, height = 8)
